...
function add_post_report()
{
    ...
    \Ratchet\Client\connect('ws://178.128.202.94:8000')->then(function ($conn) {
        ...
        try {
            ...
            report_mail_admin('mr.sn5.kma@gmail.com', $user_info->user_login, $post->post_message);
...
function get_forum_posts()
{
    ...
    try {
        foreach ($wpdb->get_results($sqlQuery, ARRAY_A) as $post) {
            ...
            $post['photo'] = ($role == 'administrator' || $role == 'adviser') ? site_logo() : get_avatar_url($post['user_id']);
